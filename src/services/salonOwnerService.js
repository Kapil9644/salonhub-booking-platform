...
export const getSalonOwnerProfile = async () => {
  const response = await salonOwnerApi.get("/salon-owner-auth/me");
  return response.data;
};

// Update logged-in salon owner's personal profile
export const updateSalonOwnerProfile = async (profileData) => {
  const response = await salonOwnerApi.put(
    "/salon-owner-auth/profile",
    profileData,
  );
  return response.data;
};
...
export const uploadSalonOwnerProfileImage = async (imageFile) => {
  ...
  const response = await salonOwnerApi.post(
    "/salon-owner-auth/profile-image",
    formData,
  );
